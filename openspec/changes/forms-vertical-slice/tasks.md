...
### [x] 1.1 Add `QualityForm` model to `apps/quality/models.py`
...
### [x] 1.2 Add `FormField` model to `apps/quality/models.py`
...
### [x] 1.3 Register models in `apps/quality/admin.py`
...
### [x] 2.3 Add `QualityFormCreateSerializer` (write) to `serializers.py`
...
### [x] 2.4 Add `QualityFormStatusSerializer` to `serializers.py`
...
### [x] 2.5 Add helper `_get_form_or_404(form_id, lot)` to `views.py`

- `QualityForm.objects.get(pk=form_id, lot=lot)` — returns None on DoesNotExist

### [x] 2.6 Add `FormListCreateView` to `views.py`
...
### [x] 2.7 Add `FormDetailView` to `views.py`

- `GET`: get membership → get lot → `_get_form_or_404` → serialize + 200 or 404

### [x] 2.8 Add `FormStatusView` to `views.py`
...
### [x] 3.1 Create `frontend/lib/forms/types.ts`
...
### [x] 3.2 Create `frontend/lib/forms/api.ts`
...
### [x] 3.3 Create `frontend/hooks/use-forms.ts`
...
### [x] 3.4 Create `frontend/components/forms/form-status-badge.tsx`
...
### [x] 3.5 Create `frontend/app/dashboard/lots/[id]/forms/page.tsx`
...
### [x] 4.1 Test `QualityForm` model creation and `__str__`
...
### [x] 4.2 Test `FormField` model creation
...
### [x] 4.3 Test `POST /api/v1/lots/{lot_id}/forms/` — success
...
### [x] 4.4 Test `POST /api/v1/lots/{lot_id}/forms/` — invalid form_type

- POST with `form_type="garbage"` → assert HTTP 400

### [x] 4.5 Test `GET /api/v1/lots/{lot_id}/forms/` — list scoped to tenant

- Create 2 forms for tenant A, 1 for tenant B
- GET as tenant A user → assert 2 results

### [x] 4.6 Test `GET /api/v1/lots/{lot_id}/forms/{form_id}/` — detail

- GET existing form → 200 with `fields` array
- GET form from wrong lot → 404

### [x] 4.7 Test `PATCH status/` — draft → submitted
...
### [x] 4.8 Test `PATCH status/` — submit blocked when fields empty

- POST form with `value: ""`
- PATCH `{status: "submitted"}` → assert HTTP 400

### [x] 4.9 Test `PATCH status/` — approve by quality_manager
...
### [x] 4.10 Test invalid status transitions
...
### [x] 5.1 Update `AGENTS.md` forms domain entry
...
### [x] 5.2 Update `README.md` API section
