...
- [ ] **T1.1** — Add `Plan` model to `backend/apps/core/models.py` — define `Plan(models.Model)`
...
- [ ] **T1.2** — Add `TenantSubscription` model to `backend/apps/core/models.py` — define
...
- [ ] **T1.3** — Generate migration for `Plan` + `TenantSubscription` — run
...
- [ ] **T1.4** — Create `backend/apps/core/management/commands/seed_plans.py` — implement
...
- [ ] **T1.5** — Register `Plan` and `TenantSubscription` in `backend/apps/core/admin.py` —
...
- [ ] **T2.1** — Create `backend/apps/billing/__init__.py` (empty) and
...
- [ ] **T2.2** — Create `backend/apps/billing/serializers.py` — define
...
- [ ] **T2.3** — Create `PlanListView(APIView)` in `backend/apps/billing/views.py` —
...
- [ ] **T2.4** — Create `SubscriptionView(APIView)` in `backend/apps/billing/views.py` —
...
- [ ] **T2.5** — Create `backend/apps/billing/urls.py` — define
...
- [ ] **T2.6** — Modify `backend/config/urls.py` — add
...
- [ ] **T3.1** — Create `frontend/lib/billing/types.ts` — export `SubscriptionStatus` type
...
- [ ] **T3.2** — Create `frontend/hooks/use-subscription.ts` — implement `useSubscription()`
...
- [ ] **T3.3** — Create `frontend/components/billing/plan-badge.tsx` — `"use client"`;
...
- [ ] **T3.4** — Create `frontend/app/dashboard/billing/page.tsx` — `"use client"`;
...
- [ ] **T4.1** — Create `backend/tests/test_billing.py` — add module docstring; import pytest,
...
- [ ] **T4.2** — In `backend/tests/test_billing.py` — write `test_plan_list_public_200`:
...
- [ ] **T4.3** — In `backend/tests/test_billing.py` — write `test_plan_list_ordered_by_price`:
  create plans with prices 49.00, 0.00, 199.00; `GET /api/v1/billing/plans/` returns items in
  price order [0.00, 49.00, 199.00].
  Done when assert passes.

- [ ] **T4.4** — In `backend/tests/test_billing.py` — write
...
- [ ] **T4.8** — In `backend/tests/test_billing.py` — write `test_seed_plans_creates_three`:
  call `call_command("seed_plans")`; assert `Plan.objects.count() == 3` and
  `Plan.objects.get(code="enterprise").max_users == 999`.
  Done when asserts pass.

- [ ] **T4.9** — In `backend/tests/test_billing.py` — write `test_seed_plans_idempotent`:
...
- [ ] **T5.1** — Update `openspec/changes/billing-licensing/design.md` — mark each row in the
...
- [ ] **T5.2** — Update `docs/analisis/estado-proyecto.md` — add entry for billing-licensing:
...
- [ ] **T5.3** — Mark all tasks complete in `openspec/changes/billing-licensing/tasks.md` after
  all tests pass — change every `- [ ]` to `- [x]`.
