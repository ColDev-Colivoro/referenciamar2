...
- [ ] **T-01** Create `backend/apps/reports/__init__.py`  
...
- [ ] **T-02** Create `backend/apps/reports/apps.py`  
  ```python
...
- [ ] **T-03** Register app in `backend/config/settings/base.py`  
...
- [ ] **T-04** Create `backend/apps/reports/views.py` — `LotSummaryView`  
...
- [ ] **T-05** Add `FormSummaryView` to `backend/apps/reports/views.py`  
...
- [ ] **T-06** Add `ActivityView` to `backend/apps/reports/views.py`  
...
- [ ] **T-07** Add `DashboardView` to `backend/apps/reports/views.py`  
...
- [ ] **T-08** Create `backend/apps/reports/urls.py`  
...
- [ ] **T-09** Wire reports URLs in `backend/config/urls.py`  
  Add:
  ```python
...
- [ ] **T-10** Verify 403 guard — no membership returns 403 on all four endpoints  
...
- [ ] **T-11** Create `frontend/lib/reports/types.ts`  
  Define and export:
...
- [ ] **T-12** Create `frontend/lib/reports/api.ts`  
...
- [ ] **T-13** Create `frontend/hooks/use-reports.ts`  
...
- [ ] **T-14** Create `frontend/components/dashboard/stats-cards.tsx`  
...
- [ ] **T-15** `test_lot_summary_counts` in `backend/tests/test_reports.py`  
...
- [ ] **T-21** Update `openspec/specs/` — move `changes/reports-base/specs/reports/spec.md` reference into main specs index if one exists (or create entry).  
  _Acceptance_: Spec is reachable from openspec root.

- [ ] **T-22** Add reports endpoints to project API reference (if `docs/api.md` or similar exists).  
